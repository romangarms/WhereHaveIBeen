 ...
 */
export function getCacheKey(user, device) {
 ...
 */
export async function getCache(user, device) {
  try {
    await initDB();
 ...
 */
export async function setCache(user, device, data) {
  try {
    await initDB();
    const key = getCacheKey(user, device);
 ...
 */
export async function clearCacheByKey(key) {
  try {
    await initDB();
 ...
 * @returns {Promise<boolean>} True if successful, false otherwise
 */
export async function clearCache(user, device) {
  const key = getCacheKey(user, device);
 ...
 */
export async function getCacheSize(user, device) {
  try {
    await initDB();
    const key = getCacheKey(user, device);
 ...
 */
export function validateCacheSettings(cachedSettings, currentBufferSize, currentOsrmUrl) {
 ...
 */
export function validateCacheData(cacheData) {
 ...
 */
export function getCacheDateRange(cacheData) {
 ...
 */
export function formatCacheDate(date) {
